  ...
  const fetchEvent = async () => {
    ...
    try {
      ...
      if (eventData.success) {
        // EVENT DATA
        setValue("id", eventData.data.event.id);
        setValue("title", eventData.data.event.title);
        setValue("description", eventData.data.event.description);
        setValue("isActive", eventData.data.event.isActive);
        setValue("startDate", new Date(eventData.data.event.startDate));
        setValue("endDate", new Date(eventData.data.event.endDate));
        // CANDIDATES DATA
        const candidatePairs: CandidatePair[] = eventData.data.event.candidates
          ? Array.from(
              new Set(
                eventData.data.event.candidates.reduce(
                  (pairs: CandidatePair[], candidate: Candidate) => {
                    // Find candidate with the same sequence
                    const matchingCandidate =
                      eventData.data.event.candidates?.find(
                        (otherCandidate: Candidate) =>
                          otherCandidate.sequence === candidate.sequence &&
                          otherCandidate.id !== candidate.id
                      );

                    // Determine chairman and vice chairman based on position
                    if (matchingCandidate) {
                      const isPairComplete =
                        (candidate.position
                          ?.toLowerCase()
                          .includes("chairman") &&
                          matchingCandidate.position
                            ?.toLowerCase()
                            .includes("vice")) ||
                      ...
                      console.log("Is Pair Complete:", isPairComplete);
                      ...
                      if (isPairComplete) {
                        ...
                        // Add unique pair based on chairman and vice chairman ID
        ...
        setValue("candidates", candidatePairs);
      }
    } catch (error: any) {
      const errorMessage =
        error.response?.data?.message ||
    ...
    return (
      <SafeAreaView className="flex-1">
        <View className="p-4">
          ...
          <View>
            <Skeleton className={"w-1/2 h-6 my-3"} />
                    ...
                    return (
                      <>
                        <Button
                          ...
                          className="ps-3 flex flex-row items-center justify-start dark:border-gray-700"
                        >
                    ...
                    <Controller
                      ...
                      render={({ field: { onChange, value } }) => (
                        <TextInput
                          ...
                          value={value}
                          placeholderTextColor="#9CA3AF"
                          className={`w-full px-3 py-2 border rounded-md dark:text-gray-100 ${
